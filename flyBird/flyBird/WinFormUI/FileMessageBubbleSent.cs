...
{
    public partial class FileMessageBubbleSent : UserControl
    {
        ...
        public FileMessageBubbleSent(FileShareMiddleController fsmc, string time, MsgType messageType)
        {
            InitializeComponent();
...
//                this.bubbleText.ForeColor = outMsgTextColor;
        ...
        private void OnTransferClientStopped(object sender, TransferQueue queue)
        {
            ...
            lstTransferView.Items[queue.ID.ToString()].Remove();
        }
...
//            i.SubItems.Add(queue.Type == QueueType.Download ? "Download" : "Upload");
        ...
        private void progressSet(TransferQueue queue)
        {
            ...
            if (lstTransferView.Items!=null)
            {
                int x = queue.Progress;

                lstTransferView.Items[queue.ID.ToString()].SubItems[2].Text = x + "%";
